...
public class CMongoQuey<E extends Serializable> {
    ...
    private List<DBObject> select() {
        ...
        if (page > 0 && pageCount > 0) {
            if (page == 1) dbCursor = collection.find(query).limit(pageCount);
            else
                dbCursor = collection.find(query).skip((page - 1) * pageCount).limit(pageCount);
        } else {
    ...
    /**
     * @param obj 可以更新一个实体或一个子文档 数据库不存在，不添加，只更新找到的第一个，不是更新全部
     * @return
     */
     ...
     *
     * @param obj 修改或增加子集 如果其操作的键不存在，则创建新的键值 作的键值已经存在，且其值为数组类型，该修改符将为该数组添加新的数组元素
     * @return
     */
